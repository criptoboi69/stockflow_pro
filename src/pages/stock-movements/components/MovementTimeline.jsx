...
const MovementTimeline = ({ 
  ...
  const getQuantityDisplay = (movement) => {
    const sign = movement?.type === 'receipt' ? '+' : '-';
    return `${sign}${Math.abs(movement?.quantity)}`;
  };
        ...
        <div key={movement?.id} className="relative">
          ...
          <div className="flex items-start space-x-3 sm:space-x-4 p-3 sm:p-4 bg-surface rounded-lg border border-border hover:border-primary/20 transition-colors">
            ...
            <div className="flex-1 min-w-0">
              {/* Product name and SKU - stack on mobile */}
              <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-2 mb-2">
                <h4 className="font-medium text-text-primary text-sm sm:text-base break-words">{movement?.product?.name}</h4>
                <span className="text-xs px-2 py-1 bg-muted rounded-full text-text-muted inline-block w-fit mt-1 sm:mt-0">
                  SKU: {movement?.product?.sku}
                </span>
              </div>
              
              {/* Date, time, user - wrap on mobile */}
              <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-text-secondary mb-2 sm:mb-3">
                ...
                <span className="flex items-center space-x-1">
                  ...
                  <span className="whitespace-nowrap">{new Date(movement.createdAt)?.toLocaleTimeString('fr-FR', { 
                    hour: '2-digit', 
                    minute: '2-digit' 
                  })}</span>
                </span>
              ...
              {/* Quantity, balance, location - stack on mobile */}
              <div className="grid grid-cols-2 sm:flex sm:items-center gap-2 sm:gap-6 text-xs sm:text-sm mb-2 sm:mb-0">
                <div className="flex items-center space-x-1 sm:space-x-2">
                  ...
                  <span className={`font-medium ${
                    movement?.type === 'receipt' ? 'text-success' : 'text-error'
                  }`}>
                ...
                <div className="flex items-center space-x-1 sm:space-x-2 col-span-2">
                  ...
                  <span className="text-text-primary truncate">{movement?.location?.name}</span>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mt-3 sm:mt-2">
                {canEdit(movement) && (
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => onEditMovement?.(movement)}
                    className="text-text-secondary hover:text-primary w-full sm:w-auto justify-center sm:justify-start"
                  >
                    <Icon name="Edit" size={14} className="mr-1 flex-shrink-0" />
                ...
                <Button
                  ...
                  className="text-text-secondary hover:text-text-primary w-full sm:w-auto justify-center sm:justify-start"
                >
                  <Icon name="Eye" size={14} className="mr-1 flex-shrink-0" />
